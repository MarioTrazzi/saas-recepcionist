...
export default function SettingsPage() {
  ...
  // Meta embedded signup state (one-click connect)
  ...
  const handleEmbeddedSignup = async () => {
    ...
    try {
      ...
      await whatsappApi.embeddedSignup(result.code)
      setShowWaSetup(false)
      qc.invalidateQueries({ queryKey: ['whatsapp-status'] })
        ...
        <div id="whatsapp-channel-section" className="card p-6">
          ...
          {!checkingWa && (!isConnected || showWaSetup) && (
            <div className="space-y-5">
              ...
              {/* Embedded Signup — primary path (one-click via Meta) */}
              ...
              <div>
                <button
                  type="button"
                  onClick={() => setShowWaManual(s => !s)}
                  className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-300 transition-colors"
                >
                  <ChevronDown className={`h-3.5 w-3.5 transition-transform ${showWaManual ? 'rotate-180' : ''}`} />
